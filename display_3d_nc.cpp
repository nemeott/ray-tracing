...
// notcurses c++ bindings for terminal printing and keyboard input
...
void get_camera_basis(const Camera& camera, Vec3& forward, Vec3& left, Vec3& down) {
	...
	down = Vec3{ 0, 1, 0 };
	left = forward.cross(down).norm();
	down = left.cross(forward).norm();
}
...
int main() {
	...
	// 	// Apply the scene changes and render
	...
	while (running) {
		...
		if (ret > 0) { // Input recieved
			Vec3 forward, left, down;
			get_camera_basis(camera, forward, left, down);
			...
			case 'a': // Move camera left
				camera.position = camera.position + left * move_step;
				break;
			case 'd': // Move camera right
				camera.position = camera.position - left * move_step;
				break;
			case NCKEY_SPACE: // Move camera up
				camera.position = camera.position - down * move_step;
				break;
			case 'x': // Move camera down
				camera.position = camera.position + down * move_step;
				break;

			case NCKEY_UP:
				camera.pitch_degrees += move_step;
			...
			case NCKEY_DOWN:
				...
				break;
			case NCKEY_LEFT:
				camera.yaw_degrees -= move_step;
				if (camera.yaw_degrees < 0.0f) camera.yaw_degrees += 360.0f;
				if (camera.yaw_degrees >= 360.0f) camera.yaw_degrees -= 360.0f;
				break;
			case NCKEY_RIGHT:
				camera.yaw_degrees += move_step;
				if (camera.yaw_degrees < 0.0f) camera.yaw_degrees += 360.0f;
